define([
  ...
  return declare([Text], {
    ...
    , saveContentPs: function(par, parContent){
      ...
      if (solString.endsWith(fName, ".less")){
        fName = fName.substr(0, fName.length - 5);
      };
      fName += ".css";
        console.log("css:" + fName);
      this.testPs(par, parContent.text).then(function(parCss){
        console.log("css result");
        files.writeTextDef(fName, parCss).then(function(){
          savePs.resolve();
        });
      });
      }catch(e){
